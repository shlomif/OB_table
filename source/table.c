...
*/
static inline struct OB_item* OB_find_helper(struct OB_table *t, struct OB_item pivot)
{
	size_t tries, i;
...
{
	struct OB_item*result = OB_find_helper(t, OB_el2pivot(t, el));
...
{
	struct OB_table tc;
	struct OB_item*loc = OB_find_helper(t, pivot);
	/* Element not exist? */
